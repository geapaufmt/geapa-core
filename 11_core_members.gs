...
function core_buildMeuPerfilPortalResponse_(sessao, bundle) {
  ...
  return Object.freeze({
    ...
    perfil: Object.freeze({
      ...
      paisOrigemCodigo: String(pessoa.PAIS_ORIGEM_CODIGO || "").trim(),
      paisOrigemNome: String(pessoa.PAIS_ORIGEM_NOME || "").trim(),
      municipioOrigemCodigo: String(pessoa.MUNICIPIO_ORIGEM_CODIGO || "").trim(),
      regiaoOrigem: String(pessoa.REGIAO_ORIGEM || "").trim(),
      cursoId: String(detalhes.CURSO_ID || "").trim(),
      cursoNome: String(detalhes.CURSO_NOME_SNAPSHOT || "").trim(),
      instituicaoEnsino: String(detalhes.INSTITUICAO_ENSINO || "").trim(),
      campus: String(detalhes.CAMPUS || "").trim(),
      nivelCurso: String(detalhes.NIVEL_CURSO || "").trim(),
      periodoIngressoCurso: String(detalhes.PERIODO_INGRESSO_CURSO || "").trim(),
      semestreAtualCursoCalculado: Number(detalhes.SEMESTRE_ATUAL_CURSO_CALCULADO || 0) || null,
      semestreAtualCursoCalculadoEm: String(detalhes.SEMESTRE_ATUAL_CURSO_CALCULADO_EM || "").trim(),
      statusCompletudeCadastral: String(detalhes.STATUS_COMPLETUDE_CADASTRAL || "PENDENTE").trim(),
      explicacaoSemestreAtualCurso: "Estimativa calculada pelo periodo de ingresso registrado no RGA e pelo calendario academico. Pode nao refletir trancamentos ou alteracoes individuais.",
